...
function getSchoolRankings(players: ReturnType<typeof getPlayers>, year: number) {
...
export default function SchoolRankingsPage() {
  ...
  const schools = getSchoolRankings(players, selectedYear);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-500 ml-4 pl-3">
          Which colleges are landing the most SD County talent?
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div className="flex rounded-xl overflow-hidden border border-gray-200">
          {([2026, 2027] as const).map(year => (
            ...
            >
              {year}
              {year === 2027 && <span className="ml-1 text-xs font-bold" style={{ color: selectedYear === 2027 ? '#FFD700' : '#CC0000' }}>● LIVE</span>}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {schools.slice(0, 3).map((s, i) => (
            <div key={s.school} className="rounded-2xl p-6 text-white" style={{ backgroundColor: '#002147' }}>
              ...
              <div className="text-blue-200 text-sm mb-3">{s.count} player{s.count !== 1 ? 's' : ''} · {s.totalStars}★ combined</div>
              <div className="space-y-1">
                {s.players.map(p => (
                  <div key={p.id} className="text-xs text-blue-200 flex items-center gap-2">
                    ...
                    <span className="text-xs px-1.5 py-0.5 rounded font-bold" style={{ backgroundColor: p.position?.includes('QB') ? '#CC0000' : '#1e3a5f' }}>
                      {p.position}
                    </span>
                  </div>
                ))}
      ...
      {schools.length > 0 ? (
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <div className="flex flex-wrap gap-1">
                      {s.players.map(p => (
                        ...
                        >
                          {'★'.repeat(p.stars || 0)} {p.firstName} {p.lastName}
                        </Link>
                      ))}
                    </div>
                  </td>
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">Check back as the recruiting cycle progresses.</p>
        </div>
      )}

      <div className="mt-8 text-center">
        <p className="text-xs text-gray-400">Rankings based on SD County prospects in our database · Class of {selectedYear}</p>
      </div>
    </div>
  );
}
